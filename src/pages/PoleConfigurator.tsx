...
function CameraModel({ type }: { type: string }) {
  ...
  switch (type) {
    default:
    case "ptz":
      // Compact adjustable wide-angle camera: small turret body with a dark recessed lens face.
      return (
        <group>
          {/* Short swivel base */}
          <mesh position={[0, 0.008, 0]} rotation={[0, 0, Math.PI / 2]}>
            <cylinderGeometry args={[0.047, 0.047, 0.026, 18]} />
            <meshStandardMaterial {...bodyM} />
          </mesh>
          <mesh position={[0.018, -0.014, 0]} rotation={[0, 0, Math.PI / 2]}>
            <cylinderGeometry args={[0.037, 0.041, 0.048, 18]} />
            <meshStandardMaterial {...bodyM} />
          </mesh>
          {/* Angled camera ball */}
          <mesh position={[0.073, -0.044, 0]}>
            <sphereGeometry args={[0.066, 22, 22]} />
            <meshStandardMaterial {...bodyM} />
          </mesh>
          <mesh position={[0.073, -0.044, 0]} rotation={[0, Math.PI / 2, 0]}>
            <torusGeometry args={[0.055, 0.004, 6, 28]} />
            <meshStandardMaterial {...trimM} />
          </mesh>
          {/* Recessed 190-degree lens face */}
          <mesh position={[0.126, -0.048, 0]} rotation={[0, Math.PI / 2, 0]}>
          ...
          <mesh position={[0.134, -0.048, 0]}>
            <sphereGeometry args={[0.018, 12, 12]} />
            <meshStandardMaterial {...lensM} />
          </mesh>
          {/* Subtle front trim screws */}
          {([-1, 1] as const).map(s => (
            <mesh key={s} position={[0.139, -0.048 + s * 0.024, 0.018]} rotation={[0, Math.PI / 2, 0]}>
              <cylinderGeometry args={[0.0035, 0.0035, 0.004, 6]} />
              <meshStandardMaterial color="#8f9499" roughness={0.38} metalness={0.8} />
            </mesh>
          ))}
        </group>
      );
...
// ─── Honeycomb Mount — LTB301 gooseneck arm ──────────────────────────────────
// Rectangular 4-hole base plate with vertical ribs, hollow curved arm, circular
// pendant cap with cable-routing centre hole. Matte Signal White powder coat.
function HoneycombMount({ poleColor, cameraType, W }: {
  poleColor: string; cameraType: string; W: number;
}) {
  const faceX = W / 2;
  const whiteM = { color: "#e6e9e9", roughness: 0.40, metalness: 0.26 } as const;
  const rimM   = { color: "#c8cbcc", roughness: 0.30, metalness: 0.38 } as const;
  const boltM  = { color: "#2a2a2a", roughness: 0.42, metalness: 0.88 } as const;
  const holeM  = { color: "#0d0d0d", roughness: 0.85, metalness: 0.10 } as const;

  // Gooseneck spline: horizontal out then sweeps downward
  const curve = useMemo(() => new THREE.CatmullRomCurve3([
    new THREE.Vector3(faceX + 0.040,  0.000, 0),
    new THREE.Vector3(faceX + 0.090,  0.008, 0),
    new THREE.Vector3(faceX + 0.140,  0.022, 0),
    new THREE.Vector3(faceX + 0.175,  0.018, 0),
    new THREE.Vector3(faceX + 0.192, -0.020, 0),
    new THREE.Vector3(faceX + 0.185, -0.080, 0),
    new THREE.Vector3(faceX + 0.165, -0.148, 0),
  ]), [faceX]);

  const tubeGeo = useMemo(() =>
    new THREE.TubeGeometry(curve, 32, 0.0165, 12, false),
  [curve]);

  // Pendant cap end-point derived from curve
  const capPos: [number, number, number] = [faceX + 0.163, -0.158, 0];

  return (
    <group>
      {/* ── Mounting base plate (rectangular, 132 mm wide) ── */}
      <mesh position={[faceX + 0.020, 0, 0]}>
        <boxGeometry args={[0.040, 0.152, 0.132]} />
        <meshStandardMaterial {...whiteM} />
      </mesh>

      {/* Vertical reinforcement ribs × 3 */}
      {[-0.044, 0, 0.044].map(z => (
        <mesh key={z} position={[faceX + 0.043, 0, z]}>
          <boxGeometry args={[0.008, 0.144, 0.010]} />
          <meshStandardMaterial {...whiteM} roughness={0.26} />
        </mesh>
      ))}

      {/* 4-hole bolt pattern (2 × 2) — recessed dark sockets */}
      {([-1, 1] as const).flatMap(sy =>
        ([-1, 1] as const).map(sz => (
          <mesh key={`s${sy}${sz}`}
            position={[faceX + 0.041, sy * 0.053, sz * 0.042]}
            rotation={[0, Math.PI / 2, 0]}
          >
            <cylinderGeometry args={[0.009, 0.009, 0.010, 8]} />
            <meshStandardMaterial {...holeM} />
          </mesh>
        ))
      )}
      {/* Bolt heads (hex) */}
      {([-1, 1] as const).flatMap(sy =>
        ([-1, 1] as const).map(sz => (
          <mesh key={`h${sy}${sz}`}
            position={[faceX + 0.044, sy * 0.053, sz * 0.042]}
            rotation={[0, Math.PI / 2, 0]}
          >
            <cylinderGeometry args={[0.006, 0.006, 0.005, 6]} />
            <meshStandardMaterial {...boltM} />
          </mesh>
        ))
      )}

      {/* ── Gooseneck hollow arm tube ── */}
      <mesh geometry={tubeGeo} castShadow>
        <meshStandardMaterial {...whiteM} />
      </mesh>

      {/* ── Pendant mounting cap (circular flange, Ø 130 mm) ── */}
      ...
      {/* Cap edge rim ring */}
      <mesh position={[capPos[0] + 0.009, capPos[1], capPos[2]]} rotation={[0, 0, Math.PI / 2]}>
        <torusGeometry args={[0.062, 0.004, 8, 32]} />
        <meshStandardMaterial {...rimM} />
      </mesh>

      {/* Centre cable-routing cutout */}
      <mesh position={[capPos[0] + 0.009, capPos[1], capPos[2]]} rotation={[0, 0, Math.PI / 2]}>
        <torusGeometry args={[0.017, 0.010, 10, 28]} />
        <meshStandardMaterial {...holeM} />
      </mesh>

      {/* 3-point mounting screws on cap face */}
      {[0, 1, 2].map(i => {
        const a = (i / 3) * Math.PI * 2 + Math.PI / 6;
        return (
          <mesh key={i}
            position={[capPos[0] + 0.009, capPos[1] + Math.sin(a) * 0.047, capPos[2] + Math.cos(a) * 0.047]}
            rotation={[0, Math.PI / 2, 0]}
          >
            <cylinderGeometry args={[0.005, 0.005, 0.007, 6]} />
            <meshStandardMaterial color="#5a5a5a" roughness={0.32} metalness={0.90} />
          </mesh>
        );
      })}

      {/* Camera hanging from pendant tip */}
      <group position={[capPos[0] + 0.022, capPos[1] - 0.018, 0]} rotation={[0.28, 0, 0]}>
        <CameraModel type={cameraType} />
      </group>
    </group>
  );
...
// ─── Circle Mount — LTB03-W deep-base junction box ───────────────────────────
// Die-cast aluminium cylinder, matte white, side-entry hexagonal conduit port,
// recessed mounting ring with 3-point screw pattern. Ø 126 mm deep base.
function CircleMountModel({ poleColor, cameraType, W }: {
  poleColor: string; cameraType: string; W: number;
}) {
  const faceX = W / 2;
  const whiteM = { color: "#e4e7e7", roughness: 0.36, metalness: 0.30 } as const;
  const rimM   = { color: "#b8bcbd", roughness: 0.28, metalness: 0.45 } as const;
  const hexM   = { color: "#d2d5d6", roughness: 0.42, metalness: 0.38 } as const;
  const boltM  = { color: "#686c6e", roughness: 0.32, metalness: 0.88 } as const;

  return (
    <group>
      {/* ── Back flange ring (pole-contact face) ── */}
      <mesh position={[faceX + 0.010, 0, 0]} rotation={[0, 0, Math.PI / 2]}>
        <cylinderGeometry args={[0.072, 0.072, 0.020, 32]} />
        <meshStandardMaterial {...rimM} />
      </mesh>

      {/* ── Main cylinder body (Ø 126 mm × 48 mm deep) ── */}
      <mesh position={[faceX + 0.034, 0, 0]} rotation={[0, 0, Math.PI / 2]}>
        <cylinderGeometry args={[0.063, 0.063, 0.048, 32]} />
        <meshStandardMaterial {...whiteM} />
      </mesh>

      {/* Subtle casting seam line around circumference */}
      <mesh position={[faceX + 0.034, 0, 0]} rotation={[0, 0, Math.PI / 2]}>
        <torusGeometry args={[0.063, 0.0018, 6, 32]} />
        <meshStandardMaterial color="#ccc" roughness={0.55} metalness={0.28} />
      </mesh>

      {/* ── Front recessed mounting ring ── */}
      <mesh position={[faceX + 0.060, 0, 0]} rotation={[0, 0, Math.PI / 2]}>
        <torusGeometry args={[0.050, 0.005, 8, 32]} />
        <meshStandardMaterial {...rimM} />
      </mesh>
      {/* Front face cap disc */}
      <mesh position={[faceX + 0.060, 0, 0]} rotation={[0, 0, Math.PI / 2]}>
        <cylinderGeometry args={[0.051, 0.051, 0.006, 32]} />
        <meshStandardMaterial {...whiteM} />
      </mesh>

      {/* 3-point mounting screws at 120° */}
      {[0, 1, 2].map(i => {
        const a = (i / 3) * Math.PI * 2 + Math.PI / 6;
        return (
          <mesh key={i}
            position={[faceX + 0.062, Math.sin(a) * 0.040, Math.cos(a) * 0.040]}
            rotation={[0, Math.PI / 2, 0]}
          >
      ...
      {/* ── Side-entry conduit port (hexagonal waterproof cable gland) ── */}
      {/* Port body — hex cross-section cylinder */}
      <mesh position={[faceX + 0.034, -0.072, 0]} rotation={[0, 0, 0]}>
        <cylinderGeometry args={[0.014, 0.014, 0.038, 6]} />
        <meshStandardMaterial {...hexM} />
      </mesh>
      {/* Gland nut (wider hex flange) */}
      <mesh position={[faceX + 0.034, -0.092, 0]}>
        <cylinderGeometry args={[0.020, 0.020, 0.014, 6]} />
        <meshStandardMaterial {...hexM} />
      </mesh>
      {/* Conduit port cap (round) */}
      <mesh position={[faceX + 0.034, -0.102, 0]}>
        <cylinderGeometry args={[0.010, 0.010, 0.010, 12]} />
        <meshStandardMaterial color="#1a1a1a" roughness={0.65} metalness={0.20} />
      </mesh>

      {/* Camera on front face */}
      <group position={[faceX + 0.082, -0.018, 0]} rotation={[0, 0, -0.06]}>
        <CameraModel type={cameraType} />
      </group>
    </group>
  );
...
// ─── Dispatch to correct mount model ─────────────────────────────────────────
function PoleDirectCameraMount({
  mountStyle, poleColor, cameraType, W,
}: {
  mountStyle: string;
  poleColor: string;
  cameraType: string;
  ...
  if (mountStyle === "honeycomb") {
    return <HoneycombMount poleColor={poleColor} cameraType={cameraType} W={W} />;
  }
  return <CircleMountModel poleColor={poleColor} cameraType={cameraType} W={W} />;
}
...
function SecurityPole({ config }: { config: PoleConfig }) {
  ...
  const cameraMountPositions: { angle: number; yOffset: number }[] = useMemo(() => {
    const n = config.cameraCount;
    if (n === 0 || !["circle", "honeycomb"].includes(config.armConfig)) return [];
    const angles = n === 1
      ? [0]
      : Array.from({ length: n }, (_, i) => (i / n) * Math.PI * 2);
    const verticalStep = config.armConfig === "honeycomb" ? 0.26 : 0.20;
    return angles.map((angle, i) => ({ angle, yOffset: -(i * verticalStep) }));
  ...
  return (
    <group ref={groupRef} position={[0, -(poleH / 2) - 0.06, 0]}>
      ...
      {/* Subtle corner highlights keep dark finishes from reading as a flat silhouette. */}
      {([-1, 1] as const).flatMap(sx =>
        ([-1, 1] as const).map(sz => (
          <mesh key={`${sx}-${sz}-edge`} position={[sx * (W / 2 + 0.001), poleH / 2 + 0.19, sz * (W / 2 + 0.001)]}>
            <boxGeometry args={[0.006, poleH * 0.98, 0.006]} />
            <meshStandardMaterial color="#2a2a2a" roughness={0.42} metalness={0.72} />
          </mesh>
        ))
      )}
      ...
      {/* Pole-direct camera mounts: circle mount or L mount, rotated around the pole. */}
      {cameraMountPositions.map((pos, i) => (
        <group
          key={i}
          position={[0, poleH - 0.08 + pos.yOffset, 0]}
          rotation={[0, pos.angle, 0]}
        >
          <PoleDirectCameraMount
            mountStyle={config.armConfig}
      ...
      {/* Small service labels on the pole face to make the model feel more engineered. */}
      {cameraMountPositions.length > 0 && (
        <group position={[W / 2 + 0.003, poleH - 0.08, -0.066]} rotation={[0, 0, 0]}>
          <mesh>
            <boxGeometry args={[0.004, 0.12, 0.022]} />
            <meshStandardMaterial color="#1b1b1b" roughness={0.62} metalness={0.35} />
          </mesh>
        </group>
      )}

      {/* ── Fallback for older saved configurations ── */}
      {!["circle", "honeycomb"].includes(config.armConfig) && Array.from({ length: config.cameraCount }, (_, i) => {
...
const STEPS = [
  ...
  { id: "mountStyle", label: "Camera Mount", icon: Layers },
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      ...
      <div className="rounded-xl p-4 mb-5 grid grid-cols-3 gap-2" style={{ background: "hsl(0 0% 97%)", border: "1px solid hsl(0 0% 91%)" }}>
        {[
          ...
          { label: "Camera Mount", val: ARM_CONFIGS.find(a => a.value === config.armConfig)?.label },
  ...
  return (
    <Layout>
      <SEOHead
        ...
        description="Design your custom security camera pole with our interactive 3D configurator. Choose height, pole-direct camera mounts, cameras, lighting, and finish. Get a free quote for your Houston-area property."
      />
      ...
      <section className="relative overflow-hidden" style={{ background: "hsl(0 0% 3%)" }}>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 lg:py-24 text-center">
          ...
          >
            Configure height, pole-direct camera mounts, cameras, lighting, and finish. See your setup in real-time 3D then connect with our team for a custom estimate — built for HOAs, commercial facilities, and property managers.
          </motion.p>
          <motion.div
            initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
            className="mx-auto max-w-2xl mb-8 rounded-2xl border px-5 py-4 text-left"
            style={{ background: "rgba(255,255,255,0.06)", borderColor: "rgba(255,255,255,0.14)" }}
          >
            <div className="flex items-start gap-3">
              <AlertTriangle className="w-5 h-5 shrink-0 mt-0.5" style={{ color: "hsl(0 85% 65%)" }} />
              <div>
                <p className="text-sm font-semibold text-white">Experimental Feature Under Construction</p>
                <p className="text-sm leading-relaxed mt-1" style={{ color: "rgba(255,255,255,0.56)" }}>
                  Visual aesthetics of our products can vary from the 3D rendering. Contact us for immediate assistance.
                </p>
              </div>
            </div>
          </motion.div>
      ...
      <section className="border-t border-gray-100 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14 grid lg:grid-cols-3 gap-8">
          {[
            ...
            { icon: Zap, title: "Integrated with Your System", body: "Poles connect to your existing NVR, VMS, or Alarm.com platform with hardwired PoE or fiber options available." },
          ].map(f => (
